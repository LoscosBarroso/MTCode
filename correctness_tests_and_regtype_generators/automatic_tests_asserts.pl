:- module(automatic_tests_asserts, _, [hiord, fsyntax, assertions, doccomments, datafacts]).
...
main_test(NumTypes,MaxClauses,BaseTypes,Functors) :-
    ...
    extra_cleanup_types,
    list_type_asserts(_), nl, %printing the asserted facts
    insert_types(Definitions),
    post_init_types,
    nl,display('post_init'), nl,
    list_type_asserts(_), nl, %printing the asserted facts
    test_pair((t1,t2)),
